...
# Custom styling (only Streamlit-supported)
st.markdown("""
<style>
    ...
    .stSuccess { background-color: #1a2a1a; }
</style>
...
# Main Tabs - Pure Streamlit widgets
...
with tab3:
    ...
    **Lemon Haze Auto Clones** — $20 each (minimum 4)
    ...
    if st.button("🛒 Reserve Clones Now", type="primary", use_container_width=True):
...
with tab4:
    ...
    if st.button("Notify Me When Edibles Launch", type="primary", use_container_width=True):
        if email:
            st.balloons()
...
st.caption("Built with Streamlit 1.42.0")
